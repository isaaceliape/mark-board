...
// Mock file system API factory
const createMockFsAPI = () => ({
  readdir: jest.fn(),
  readFile: jest.fn(),
  writeFile: jest.fn(),
  unlink: jest.fn(),
...
let mockFsAPI: ReturnType<typeof createMockFsAPI>

beforeEach(() => {
  // Create fresh mock API for each test
  mockFsAPI = createMockFsAPI()
  setFileSystemAPI(mockFsAPI)
})

describe('fileOperations', () => {
  // Outer describe for organization
})
...
describe('writeCard', () => {
  it('should serialize and write a card to file', async () => {
    ...
    const [filePath, content] = mockFsAPI.writeFile.mock.calls[0]
    expect(filePath).toBe('./kanban-data/backlog/test-card.md')
    expect(content).toContain('title: Test Card')
    expect(content).toContain("created: '2023-01-01T00:00:00.000Z'")
...
describe('ensureColumnsExist', () => {
  it('should create directories for all columns', async () => {
    ...
    expect(mockFsAPI.mkdir).toHaveBeenCalledWith('./kanban-data/in-progress', {
      recursive: true,
    })
