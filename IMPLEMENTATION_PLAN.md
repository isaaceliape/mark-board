...
- [ ] Set up project structure (src/, kanban-data/, etc.)
...
npm install chokidar gray-matter remark remark-parse
npm install zustand
...
- [ ] Parse markdown files with frontmatter extraction
- [ ] Convert frontmatter to Card metadata
...
- [ ] Write card data to markdown file
- [ ] Move files between folders (column changes)
- [ ] Delete markdown files
- [ ] Create new markdown files with template
- [ ] Handle file naming conflicts

### 2.3 File Watcher Hook (`src/hooks/useFileWatcher.ts`)
- [ ] Initialize chokidar watcher on kanban-data/
- [ ] Listen for add/change/unlink events
- [ ] Debounce rapid file changes
- [ ] Update store on file system changes
- [ ] Handle watcher cleanup on unmount

## Phase 3: State Management (Week 2)

### 3.1 Board Store (`src/stores/boardStore.ts`)
- [ ] Define Card and Column interfaces
- [ ] Create Zustand store with initial state
- [ ] Implement actions:
  - [ ] loadCards() - Initialize from file system
  - [ ] addCard(card) - Create new card
  - [ ] updateCard(id, updates) - Edit card content
  - [ ] moveCard(id, targetColumn) - Move between columns
  - [ ] deleteCard(id) - Remove card
  - [ ] syncFromFileSystem() - Handle external changes
...
- [ ] Display card title and preview
- [ ] Show metadata badges (tags, assignee, due date)
- [ ] Implement click-to-edit mode
...
- [ ] Inline markdown editor with textarea
- [ ] Live markdown preview toggle
- [ ] Metadata form fields (tags, assignee, due date)
- [ ] Save/Cancel buttons
...
- [ ] Display card count
- [ ] Render cards in vertical list
- [ ] Add "New Card" button
- [ ] Implement drop zone styling
- [ ] Handle empty state

### 4.4 Board Component (`src/components/Board.tsx`)
- [ ] Render all columns horizontally
- [ ] Implement @dnd-kit drag and drop context
...
- [ ] Configure @dnd-kit sensors (mouse, touch, keyboard)
- [ ] Implement handleDragStart callback
- [ ] Implement handleDragEnd callback
- [ ] Optimistic UI updates
- [ ] Rollback on file operation failure
- [ ] Visual feedback during drag

### 5.2 Integration with File System
- [ ] Move markdown file to new folder on drop
- [ ] Update card metadata with new column
